    ...
    public class BenzingaNewsJsonConverter : JsonConverter
    {
        ...
        public static BenzingaNews DeserializeNews(JToken item, bool enableLogging = false)
        {
            var instance = new BenzingaNews
            {
                ...
                CreatedAt = Parse.DateTime(item.Value<String>("created")).ToUniversalTime(),
                // CreatedAt = item.Value<DateTime>("created").ToUniversalTime(),
                UpdatedAt = Parse.DateTime(item.Value<String>("updated")).ToUniversalTime(),
            ...
            if (item["stocks"] != null)
            {
                ...
                foreach (var tickerJToken in JArray.Parse(item["stocks"].ToString()))
                {
                    ...
                    var tickerString = tickerJToken.Value<string>("name").Trim().Replace('-', '.').Replace('/', '.');
                    ...
                    if (!ShareClassMappedTickers.ContainsKey(tickerString))
                    {
                        // SecurityIdentifier sid = SecurityIdentifier.GenerateEquity(symbolTicker, QuantConnect.Market.USA, mapSymbol: false, mappingResolveDate: instance.CreatedAt);
                        DateTime createdAt = instance.CreatedAt;
                        // SecurityIdentifier sid = SecurityIdentifier.GenerateEquity(symbolTicker, Market.USA, mapSymbol: true, mappingResolveDate: createdAt);
                        
                        SecurityIdentifier sid = SecurityIdentifier.GenerateEquity(tickerString, Market.USA, mapSymbol: false, mappingResolveDate: createdAt);
                        tempSymbols.Add(new Symbol( sid, tickerString ));

                        // Symbol symbol = Symbol.Create(tickerString, SecurityType.Equity, Market.USA);
                    ...
                    else
                    {
                        if (enableLogging)
                        {
                            Log.Trace($"BenzingaNewsJsonConverter.DeserializeNews(): Ticker {tickerString} will be added as: {string.Join(",", ShareClassMappedTickers[tickerString])}");
                        }
                        foreach (var mappedTicker in ShareClassMappedTickers[tickerString])
                        {
